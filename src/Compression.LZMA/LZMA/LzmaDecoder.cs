...
namespace SevenZip.LZMA
{
    public class Decoder : ICoder, ISetDecoderProperties // ,System.IO.Stream
    {
        ...
        public Decoder()
        {
            ...
            for (int i = 0; i < Base.KNumLenToPosStates; i++) m_PosSlotDecoder[i] = new BitTreeDecoder(Base.KNumPosSlotBits);
        }
        ...
        public void Code(Stream inStream, Stream outStream, Int64 inSize, Int64 outSize, ICodeProgress progress)
        {
            ...
            {
                if (m_IsMatchDecoders[state.Index << Base.KNumPosStatesBitsMax].Decode(m_RangeDecoder) != 0) throw new DataErrorException();
                state.UpdateChar();
        ...
        private class LiteralDecoder
        {
            ...
            private uint GetState(uint pos, byte prevByte) { return ((pos & m_PosMask) << m_NumPrevBits) + (uint) (prevByte >> (8 - m_NumPrevBits)); }
            ...
            private struct Decoder2
            {
                ...
                public byte DecodeWithMatchByte(RangeCoder.Decoder rangeDecoder, byte matchByte)
                {
                    ...
                    do
                    {
                        ...
                        if (matchBit != bit)
                        {
                            while (symbol < 0x100) symbol = (symbol << 1) | m_Decoders[symbol].Decode(rangeDecoder);
                            break;
                        }
                    } while (symbol < 0x100);
                    return (byte) symbol;
